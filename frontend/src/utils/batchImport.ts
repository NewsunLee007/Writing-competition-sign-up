...
const getExcelRowNumber = (row: Record<string, unknown>, fallbackIndex: number) => {
  const rowNum = (row as { __rowNum__?: number }).__rowNum__
  return typeof rowNum === 'number' ? rowNum + 1 : fallbackIndex + 2
}
const isMeaningfulRow = (row: Record<string, unknown>) =>
...
export const parseBatchImportFile = async (file: File): Promise<ImportedStudent[]> => {
  ...
  const rawRows = XLSX.utils.sheet_to_json<Record<string, unknown>>(sheet, { defval: '', blankrows: false })
  const rows = rawRows.filter((row) => isMeaningfulRow(row))
  ...
  return rows.map((row: Record<string, unknown>, index: number) => {
    const excelRowNumber = getExcelRowNumber(row, index)
    ...
    if (!unitType) {
      throw new Error(`第 ${excelRowNumber} 行“报名类别”无效：${typeText || '空值'}`)
    }

    const unit = resolveUnit(unitText)
    if (!unit) {
      throw new Error(`第 ${excelRowNumber} 行“学区/直属学校”无效：${unitText || '空值'}`)
    }

    if (unit.type !== unitType) {
      throw new Error(`第 ${excelRowNumber} 行“报名类别”与“学区/直属学校”不匹配`)
    }

    if (!studentName || !guideTeacher || !teamTeacherName || !teamTeacherPhone) {
      throw new Error(`第 ${excelRowNumber} 行存在必填项未填写`)
    }

    if (!PHONE_REGEX.test(teamTeacherPhone)) {
      throw new Error(`第 ${excelRowNumber} 行“带队教师电话”必须为 11 位手机号`)
    }
    ...
    if (!school) {
      throw new Error(`第 ${excelRowNumber} 行“学校”不能为空`)
    }
